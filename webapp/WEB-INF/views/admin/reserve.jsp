						...
						<c:choose>
							...
							<c:forEach var="i" begin="${map.startPage }" end="${map.startPage + map.pageLimit - 1 }" step="1">
								<c:choose>
									...
									<c:otherwise>
										<li><a href="${pageContext.servletContext.contextPath }/admin/reserve/${i }">${i }</a></li>			
									</c:otherwise>
								</c:choose>
							</c:forEach>
						<c:choose>
							<c:when test="${map.endPage lt map.page.totalPages}">
								<li><a href="${pageContext.servletContext.contextPath }/admin/reserve/${map.startPage + 5 }">▶</a></li>
							</c:when>
						</c:choose>
					</ul>
				</div>
